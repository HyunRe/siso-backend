...
- **시니어 간편 인증:** 카카오 OAuth2 + JWT 기반 무상태 인증 및 RTR(Refresh Token Rotation)을 적용한 Access/Refresh Token 관리
- **AI 가중치 매칭:** 관심사(30%), 나이(20%), MBTI(15%), 지역(15%), 활동성(10%), 습관(10%)의 6가지 지표를 기반으로 매칭 점수를 산출하고, RabbitMQ 비동기 연산 및 Redis 캐싱으로 반복 연산 부하 최소화
- **실시간 STOMP 채팅 & 알림:** 온라인 유저는 STOMP로 실시간 메시지를 전달하고, 오프라인 유저는 FCM 푸시로 분기 처리하며 RabbitMQ Consumer 병렬 처리로 메시지 처리 효율 향상
- **선(先) 음성 통화 (Agora RTC):** 1:1 음성 통화의 요청·수락·종료 상태를 관리하고, 통화 품질 모니터링을 위한 패킷 손실률 및 RTT 메트릭 수집과 통화 종료 후 채팅방 생성 연동
